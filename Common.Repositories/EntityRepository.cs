...
{
    public abstract class Repository<T> : RepositoryBase, IRepository<T>
        where T : class, IEntity
    {
        ...
        */
        ...
        private void InsertOrUpdate(IEntity entity, bool submit, IEntity parent)
        {
            ...
            foreach (var p in props.Where(x => x.PropertyType.GetInterfaces().Contains(typeof(IEntity))))
            {
                ...
                if (innerEntity != null)
                    InsertOrUpdate(innerEntity, false, entity);
            }

            foreach (var p in props.Where(x => x.GetValue(entity) != null 
                && x.PropertyType.IsGenericType 
                && (parent == null || !x.PropertyType.GetGenericArguments().Contains(parent.GetType()))
                && (typeof(ICollection<>)).IsAssignableFrom(x.PropertyType.GetGenericTypeDefinition())))
            {

                var entCol = p.GetValue(entity) as IEnumerable<IEntity>;

                if (entCol != null)
                    InsertOrUpdateCollection(entCol, false, entity);
            }
        ...
        {
            InsertOrUpdate(entity, true, null);
        }

        private void InsertOrUpdateCollection(IEnumerable<IEntity> collection, bool submit, IEntity parent)
        {
            foreach (var entity in collection)
            {
                InsertOrUpdate((IEntity)entity, false, parent);
            }
            if (submit)
                DataContext.SaveChanges();
        }
        ...
        public virtual void InsertOrUpdateCollection(ICollection<T> collection)
        {
            ...
            InsertOrUpdateCollection(collection, true, null);
        }
        ...
        public virtual void Delete(Expression<Func<T, bool>> predicate)
        {
            ...
            var objectSet = DataContext.Set<T>().Where(predicate);
